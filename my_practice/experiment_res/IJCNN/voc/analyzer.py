...
def compare_layer_ratio_method(paths, file):
    ...
    for path in paths:
        ...
        agg_salgl_path = os.path.join('agg_salgl', os.path.join(path, file))
        agg_salgl_data = pd.read_csv(agg_salgl_path)
        ...
        kmeans__mAP = kmeans_data['mAP']
        agg_salgl_mAP = agg_salgl_data['mAP']
        salgl_mAP = salgl_data['mAP']

        plt.plot(kmeans_data[x_axis][:show_epochs], kmeans__mAP[:show_epochs], 'g')
        plt.plot(agg_salgl_data[x_axis][:show_epochs], agg_salgl_mAP[:show_epochs], 'b')
        plt.plot(salgl_data[x_axis][:show_epochs], salgl_mAP[:show_epochs], 'r')
        plt.xlabel(x_axis)
        plt.ylabel('valid mAP')

        plt.legend(['kmeans', 'agg salgl', 'salgl'])

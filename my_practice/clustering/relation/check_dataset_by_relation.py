...
def calculate_plist_and_entropy():
    adjLists = []
    # Todo: 初始化json_file
    json_files = []
    num_labels = 80
    ...
    # Todo: 这里观察每个标签对的熵
    # 把概率向量存储下来
    plist_name = 'p_list.npy'
    np.save(plist_name, pList)
    entropies_name = 'entropies.npy'
    np.save(entropies_name, entropies)
    # 把熵存储下来


# calculate_plist_and_entropy()

# file_name = '../../state/p_list.npy'
# p_list = np.load(file_name)
# for i in range(len(p_list)):
#     p = p_list[i]

file_name = '../../state/entropies.npy'
entropies = np.load(file_name)
cnt = 0

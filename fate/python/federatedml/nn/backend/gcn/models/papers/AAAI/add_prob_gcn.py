...
class DynamicGraphConvolution(nn.Module):
    # 节点的输入特征
    # 节点的输出特征
    def __init__(self, in_features, out_features, num_nodes, adjList=None, label_prob_vec=None):
    ...
    def forward(self, x, out1=None, prob=True, gap=False):
        """ D-GCN module
        ...
        # 计算对dynamic_adj的一个概率向量损失
        dynamic_adj_loss = torch.tensor(0.).to(out1.device)
        out1 = nn.Sigmoid()(out1)
        if prob:
            if self.label_prob_vec is None:
                # Todo: 注意应该是dynamic_adj还是dynamic_adj的转置呢？

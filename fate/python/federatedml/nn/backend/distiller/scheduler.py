...
from federatedml.nn.backend.distiller.policy import PolicyLoss, LossComponent
...
class CompressionScheduler(object):
    ...
    def __init__(self, model, zeros_mask_dict=None, device=torch.device("cuda")):
        ...
        self.policies = {}
        self.sched_metadata = {}
        self.zeros_mask_dict = zeros_mask_dict
    ...
    def on_epoch_begin(self, epoch, optimizer=None, **kwargs):
        if self.zeros_mask_dict is None:
            self.zeros_mask_dict = create_model_masks_dict(self.model)
    ...
    def on_minibatch_end(self, epoch, minibatch_id, minibatches_per_epoch, optimizer=None):
        ...
        # the mask only if the policy is actually using the mask.
        # Todo: 再次进行掩膜操作，意思是永久进行剪枝；然而，如果是Dynamic Surgery的配置应该如何？
    ...
    def on_aggregate_end(self, epoch):
        for policy in self.policies.get(epoch, list()):
            meta = self.sched_metadata[policy]
            meta['current_epoch'] = epoch
            policy.on_aggregate_end()

    def mask_all_weights(self, is_forward=True):
        for name, param in self.model.named_parameters():
            if name not in self.zeros_mask_dict:
                return
            masker = self.zeros_mask_dict[name]
            if is_forward or not masker.mask_on_forward_only:
                # When we mask on forward-pass only, we allow the gradients to change the weights.

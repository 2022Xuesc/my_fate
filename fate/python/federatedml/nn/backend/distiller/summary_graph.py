...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True):
        ...
        with torch.onnx.set_training(model_clone, False):
            ...
            dummy_input = utils.convert_tensors_recursively_to(dummy_input, device=device)
            ...
            for node in graph.nodes():
                ...
                module_name = utils.denormalize_module_name(self._src_model, module_name)
    ...
    def predecessors(self, node, depth, done_list=None, denorm_names=True):
        ...
        if denorm_names:
            ret = [utils.denormalize_module_name(self._src_model, x) for x in ret]
        return ret
    ...
    def predecessors_f(self, node_name, predecessors_types, done_list=None, logging=None, denorm_names=True):
        ...
        if node_is_an_op:
            ...
            if node['type'] in predecessors_types and len(done_list) > 1:
                return [utils.denormalize_module_name(self._src_model, node_name) if denorm_names else node_name]
    ...
    def named_params_layers(self):
        for param_name, param in self._src_model.named_parameters():
            ...
            normalized_layer_name = utils.normalize_module_name(
    ...
    def _dedicated_module_check(self, n, dedicated_modules_only=False):
        ...
        return len(self.module_ops_map[module_name]) == 1 and not utils.has_children(module)
    ...
    def adjacency_map(self, dedicated_modules_only=False):
        ...
        for op_name, op in self.ops.items():
            ...
            def op_meta(n):
                return OpSimpleMetadata(utils.denormalize_module_name(self._src_model, n), self.ops[n]['type'])

...
def build_aggregator(param: GCNParam, init_iteration=0):
    ...
    # Todo: 将同步聚合方式改为异步聚合方式
    # fed_aggregator = SyncGCNFedAggregator(context)
    fed_aggregator = AsyncGCNFedAggregator(context)
...
# 异步聚合器
class AsyncGCNFedAggregator(object):
...
class SyncGCNFedAggregator(object):
    def __init__(self, context: FedServerContext):
        self.context = context

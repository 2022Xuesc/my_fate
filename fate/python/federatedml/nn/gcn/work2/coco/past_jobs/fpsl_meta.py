...
class FedServerContext(_FedBaseContext):
    ...
    # Todo: 先放在这里，应该没有太大的意义，因为最终求的是直接平均，而不是weight的加权平均
...
class MultiLabelFitter(object):
    ...
    def train(self, train_loader, model, criterion, optimizer, epoch, device, scheduler):
        ...
        for (_, (support_features, support_target)), (
                _, (query_features, query_target)) in zip(
            ...
            # 计算模型输出
            support_output = clone(support_features)
            ...
            # 计算模型输出
            query_output = clone(query_features)

            self.ap_meter.add(query_output.data, query_target)
        ...
        metrics = [OP, OR, OF1, CP, CR, CF1, OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, mAP.item(), support_loss, query_loss]

...
class GCNFitter(object):
    ...
    def train(self, train_loader, model, criterion, optimizer, epoch, device, scheduler):
        ...
        for train_step, ((features, inp), target) in enumerate(train_loader):
            ...
            # 计算模型输出
            cnn_predicts, gcn_predicts, dynamic_loss = model(features)
            ...
            overall_loss = asym_loss + dynamic_loss

            losses[OVERALL_LOSS_KEY].add(overall_loss.item())
            losses[DYNAMIC_ADJ_LOSS].add(dynamic_loss.item())
        ...
        metrics = [OP, OR, OF1, CP, CR, CF1, OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, mAP.item(), asym_loss,
                   dynamic_adj_loss, overall_loss]
    ...
    def validate(self, valid_loader, model, criterion, epoch, device, scheduler):
        ...
        with torch.no_grad():
            for validate_step, ((features, inp), target) in enumerate(valid_loader):
                ...
                cnn_predicts, gcn_predicts, _ = model(features)
                predicts = (cnn_predicts + gcn_predicts) / 2
                # Todo: 将计算结果添加到ap_meter中
                self.ap_meter.add(predicts.data, target)
...
def _init_gcn_learner(param, device='cpu', adjList=None, label_prob_vec=None):
    ...
    in_channel = 1024
    # 仅仅使用初始化权重，仍要进行学习
    model = aaai_add_prob_gcn(param.pretrained, adjList,
